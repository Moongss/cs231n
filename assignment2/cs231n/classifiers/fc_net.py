...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        X2, cache_relu = affine_relu_forward(X, W1, b1)
        scores, cache_relu_2 = affine_forward(X2, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        #assignment1 참고. 비슷했던 것 같다..
        loss, sf_grad = softmax_loss(scores, y)
        loss += 0.5 * self.reg * ( np.sum(W1 * W1) + np.sum(W2 * W2) )

        dx2, dw2, db2 = affine_relu_backward(sf_grad, cache_relu_2)
        dx, dw, db = affine_relu_backward(dx2, cache_relu)
