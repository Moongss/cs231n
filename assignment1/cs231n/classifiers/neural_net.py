...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    L1 = X.dot(W1) + b1          #shape is (N, H)
    L1_relu = np.maximum(L1, 0) 
    scores = L1_relu.dot(W2) + b2   #shape is (N, C)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #softmax /= np.sum(np.exp(scores), axis = 1, keedims = True) 와 동일
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      loss, gradients = self.loss(X_batch, y=y_batch, reg=reg)
      ...
      #########################################################################
      # self.params에 있는 dict가지고 해당 grad로 SGD 구하기
